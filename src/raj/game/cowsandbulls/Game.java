...
public class Game extends Activity {
	...
	private final int MAX_GUESS = 25;
    ...
    List<HashMap<String,String>> mGuessList = new ArrayList<HashMap<String,String>>();
    SimpleAdapter mGuessListAdapter;
    
    // Game Variables
    private int mCurrentGuessCount;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mGuessListAdapter = new SimpleAdapter(getApplicationContext(), mGuessList, R.layout.guessitem, mFrom, mTo);
        mGuessListView.setAdapter(mGuessListAdapter);
	...
	public void onGuessButtonClick(View view) {
		...
		// Continue Game
		if(mCurrentGuessCount < MAX_GUESS){
			String guess = mUserGuessEditText.getText().toString();
			...
			if(guess.equalsIgnoreCase(mGameWord)){
				...
				for (int i = 0; i < guess.length(); i++){
					for(int j = 0; j < mGameWord.length(); j++){
						if(i == j && guess.substring(i, i+1).equalsIgnoreCase(mGameWord.substring(j, j+1))) {
							bulls++;
						}
						else if(guess.substring(i, i+1).equalsIgnoreCase(mGameWord.substring(j, j+1))){
							cows++;
						}
					}
				}
				...
				hashMap.put("cow", Integer.toString(R.drawable.cow));
				hashMap.put("cowcount", Integer.toString(cows));
				hashMap.put("bull", Integer.toString(R.drawable.bull));
				hashMap.put("bullcount", Integer.toString(bulls));
				hashMap.put("guess", guess);
				mGuessList.add(hashMap);
    ...
    private void initializeGame(){
    	mCurrentGuessCount = 0;
    	mGameWord = getGameWord();
    	mUserGuessEditText.setText("");
    	mGuessList.clear();
    	mGuessListAdapter.notifyDataSetChanged();
    }
